...
def unify_boxcoords(box):
    # Adjust box coord to upper-left point + bottom-right point.
    x1, y1, x2, y2 = box[:, 0], box[:, 1], box[:, 2], box[:, 3]
    box[:, 0] = torch.min(x1, x2)
    box[:, 1] = torch.min(y1, y2)
    box[:, 2] = torch.max(x1, x2)
    box[:, 3] = torch.max(y1, y2)
    return box


def box_iou(box1, box2, grad_needed=False, CIoU=True):
    """
