...
class AvgPoolStride1(nn.Module):
    ...
    def forward(self, x):
        padded_x = F.pad(x, (0, self.pad, 0, self.pad), mode="replicate")
        pooled_x = nn.AvgPool2d(self.kernel_size, stride=self.stride, padding=self.pad)(padded_x)
        return pooled_x
...
def create_modules(blocks):
    ...
    for x in blocks:
        ...
        #shortcut corresponds to skip connection
            ...
            stride = int(x["stride"]) if 'stride' in x.keys() else 1
            size = int(x["size"]) if 'size' in x.keys() else 1
            if stride != 1:
                avgpool = nn.AvgPool2d(size, stride)
            else:
                avgpool = AvgPoolStride1(size)
        ...
        # Yolo is the detection layer
            ...
            module.add_module("softmax_{}".format(index), softmax_layer)

        else:
            print("Something I dunno")
            assert False, 'Unkown type: %s' % x['type']
    ...
    def forward(self, x, CUDA):
        ...
        for i in range(len(modules)):
            ...
            if (module_type == "convolutional" or module_type == "upsample" or
                    module_type == "maxpool" or module_type == 'avgpool'):
                ...
                if (layers[0]) > 0:
                    layers[0] = layers[0] - i
                layers = [layer % i - i if layer > 0 else layer for layer in layers]
                ...
                if len(layers) == 1:
                    ...
                    for layer in layers[1:]:
                        mapx = outputs[i + layer]
                        x = torch.cat((x, mapx), 1)
                ...
                outputs[i] = outputs[i - 1]  # Why not keep output of this layer like the other layers?

            elif module_type == 'softmax':
                # Do nothing for yolov3
                outputs[i] = x
            else:
                assert False, 'Unknown module type: %s' % module_type
